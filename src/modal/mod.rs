...
pub fn get_modal(ty: PopupMessage) -> (String, String) {
    match ty {
        ...
        PopupMessage::MenuHelp => ("Help".to_string(), "The first four boxes are for filling in data regarding your session. Each of these boxes must be filled in. The three toggles following allow you to select which measurement types you want collected. You must select at least one data type. The last three text boxes allow you to choose where you would like your data saved. For every data type you select measurement for, you must specify a file path for it to write to. Click submit when you're done entering your data.".to_string()),
